...
if(signupForm) {
  signupForm.addEventListener('submit', e => {
    ...
    socket.emit('signup', {signup: data})
    addAttributes(btnSubmit, {disabled: ''})
    socket.on('response', res => {
      ...
      if (r.type === 'ok') {
        ...
        signupForm.reset()
        // TODO REMOVER ATRIBUTO DISABLED 
      } else {
        alertType = 'danger'
        alertTitle = '¡Error!'
      }
      messages.innerHTML = `<div class="alert mt-3 alert-${alertType} alert-dismissible fade show" role="alert">
      <strong>${alertTitle} </strong>${r.message}
